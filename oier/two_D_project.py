...
class Particle_config:
    ...
    def cold_start_dense_slab(self):
        """
        Create cold start by packing all particles 
        into a dense square block in the bottom-left corner of the box.
        """
       
        spacing = self.sigma_0 * 1.05 
        side_count = int(np.ceil(np.sqrt(self.N)))
        ...
        for i in range(self.N):
            ...
            # Place particles in a tight grid starting from the origin
            x_vals[i] = col * spacing
            y_vals[i] = row * spacing
